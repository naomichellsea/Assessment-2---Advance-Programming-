...
#Third page for letting the users explore random meal
    def randommeal(self):
        ...
        self.favbtn = tk.Button(self.mealframe, text="My Favourite", font=("Helvetica", 12), bg="#6b0707", state="disabled", command=self.myfav)
        self.favbtn.pack(side="right", padx=20, pady=10)
    ...
    def searchclicked(self):
        ...
        if mealname:
            ...
            self.favbtn.config(state="normal")  #enable the add to favourties button from disable.
        else:
            messagebox.showerror("Failed to make you a meal ;< ")

    def myfav(self, mealname):
        #adds the current meal to favourites lists
        if hasattr(self, "mealname") and self.mealname:  #ensures meal name exits in the frame
...
#4th page for user to choose their desired type of dishes.
    ...
    def dishesresultspg(self, meals, category):
        ...
        dishescard = []
        #meal cards and display in grid layout form
        for i, meal in enumerate(meals):
            row = i // columns
            col = i % columns
            card = tk.Frame(self.root, bg=self.card_color, relief="raised", bd=2, highlightthickness=1)  #creates the card of meal
            card.grid(row=row, column=col, padx=10, pady=10)
            card.configure(highlightbackground="#ccc", highlightcolor="#ccc", relief="flat") #rounded corners with shadow
            card["borderwidth"] = 2  #visble border of the card
            label = tk.Label(card, text=meal['strMeal'], bg=self.card_color, font=("Helvetica", 14, "bold")) 
            label.pack(side="top", pady=10)
            btn = tk.Button(card, text="Explore", bg="#6b0707", fg="white", font=("Helvetica", 12), command=lambda meal=meal: self.explorebutton(meal))
            btn.bind("<Button-1>", lambda e, cmd=self.categorypage: [self.buttonsound(), cmd()])
            btn.pack(side="bottom", pady=10)
            dishescard.append(card)  #save references for later use
